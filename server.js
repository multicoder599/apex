...
function sendTelegramMessage(message) {
    ...
    axios.post(url, { chat_id: TELEGRAM_CHAT_ID, text: message, parse_mode: 'HTML' }).catch(()=>{});
}
...
mongoose.connect(MONGO_URI)
  .then(() => {
      ...
      initVirtualsEngine(); // Start Virtuals Engine upon DB connection
  })
...
app.post('/api/deposit', async (req, res) => {
    try {
        ...
        const payload = { api_key: "MGPY26G5iWPw", email: "kanyingiwaitara@gmail.com", amount: amount, msisdn: formattedPhone, callback_url: `${APP_URL}/api/megapay/webhook`, description: "ApexBet Deposit", reference: reference };
    ...
    try {
        ...
        if (user.referredBy) {
            ...
            if (referrer) {
                ...
                sendPushNotification(referrer.phone, "Referral Bonus! 🎁", `Your friend made a deposit! KES 50 added to your Bonus Wallet.`, "bonus");
            }
        }
    } catch (err) {}
});
...
app.get('/api/test', (req, res) => { res.json({ success: true, message: "Server is awake!" }); });
...
async function initVirtualsEngine() {
    try {
        ...
        if (dbState && dbState.rounds && dbState.rounds.length > 0) {
            ...
            // If server slept and season finished > 2 mins ago, reset.
...
function generateVirtualEvents(homeProb) {
    ...
    for(let min = 1; min <= 90; min++) {
        if(Math.random() < 0.032) { // Goal event
...
// 🟢 PUBLIC API ENDPOINT FOR CLIENT SYNC
...
// ==========================================
// BACKGROUND BET SETTLEMENT (SPORTS & JACKPOT ONLY)
// ==========================================
setInterval(async () => {
    try {
